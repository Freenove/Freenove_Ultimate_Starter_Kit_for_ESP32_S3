...
Download the code to ESP32-S3 WROOM, open the serial port monitor, set the baud rate to 115200, press the keyboard, the value of the pressed keys will be printed out via the serial port. As shown in the following figure: 
...
.. py:function:: class Keypad	
    ...
    :red:`You need to add the library each time you use the Keypad.`
...
|                   |                  |                                            |
| |Chapter22_08|    | |Chapter18_00|   | |Chapter22_00|                             |
+-------------------+------------------+--------------------------------------------+
| NPN transistorx1  | Active buzzer x1 | Resistor 1kΩ x1                            |
|                   |                  |                                            |
| (S8050)           |                  |                                            |
|                   |                  |                                            |
| |Chapter07_00|    | |Chapter07_01|   | |Chapter07_11|                             |
+-------------------+------------------+--------------------------------------------+

.. |Chapter22_08| image:: ../_static/imgs/22_Matrix_Keypad/Chapter22_08.png
...
Verify and upload the code to the ESP32-S3 WROOM and press the keypad to input password with 4 characters. If the input is correct, the servo will move to a certain degree, then return to the original position. If the input is wrong, an input error alarm will be generated.
...
Third, if the button has been pressed for four times, ESP32-S3 begins to judge if the password is correct.
