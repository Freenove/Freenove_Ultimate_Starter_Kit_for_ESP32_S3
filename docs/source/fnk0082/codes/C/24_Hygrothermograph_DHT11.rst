...
We use the third party library DHTesp. If you haven't installed it yet, please do so now. The steps to add third-party libraries are as follows: open arduino->Sketch->Include library-> Manage libraries. 

Enter " DHT " in the search bar and select "DHT sensor library for ESPx" for installation.
...
Compile and upload the code to the ESP32-S3 WROOM, turn on the serial monitor, and set the baud rate to 115200. Print out data of temperature and humidity sensor via the serial port.
